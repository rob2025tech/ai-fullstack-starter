...
## WO-015: User Story: WO-015 - Add mobile agent replay workflow
- **Status:** completed
- **Commit:** `f2ab393`
- **Files:** 4 (+1452/-47)
- **Duration:** 425ss
- **Approach:** Extended apps/mobile/lib/api.ts with five typed JSON-only agent API helpers (createAgentSession, getAgentSession, submitAgentTask, submitAgentApproval, getAgentEvents) following the existing BASE_URL/ContractError/errorFromResponse pattern. No SSE — getAgentEvents is the sole event source (ADR-006). Rewrote App.tsx to add an AgentScreen component beside the preserved ChatScreen, using a Chat/Agent tab switcher at the top. AgentScreen manages session create/resume, task submission, explicit Refresh timeline button, ordered event replay from getAgentEvents, pending approval cards with approve/reject actions (calls submitAgentApproval then refreshTimeline), and a terminal result section for completed/error events. Extracted mergeEvents (deduplication by event_id, sort by sequence) and extractMobileApproval as exported helpers. Created deterministic fixtures and extended api.test.ts with tests for all 5 helpers, ContractError mapping from the approval endpoint, after_sequence query param, workflow sequencing assertions, and fixture completeness checks.
