...
def test_answer_updates_misconception_and_mastery():
...
def test_learning_state_is_reused_between_answers():
    repository = InMemoryLearningRepository()
    service = LearningService(repository)
    now = datetime(2026, 9, 12, 10, 0, tzinfo=timezone.utc)
...
def test_different_students_have_independent_learning_progress():
    repository = InMemoryLearningRepository()
    service = LearningService(repository)
    now = datetime(2026, 9, 12, 10, 0, tzinfo=timezone.utc)
